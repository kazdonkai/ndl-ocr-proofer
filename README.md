...
**古文書・古典籍 OCR の対照校訂ブラウザ版**

![kokkai_民事判決録_第百六十八号_入倉森苧政事件_p02](https://github.com/user-attachments/assets/951a03a3-4e25-4c56-bf52-bcf3cd0c4268)

出典:
『大審院民事判決録 自明治十三年七月至明治十三年九月』, 司法省, 1881.  
国立国会図書館デジタルコレクション  
<https://dl.ndl.go.jp/pid/2938700> （参照 2026-06-17）

日本語 | [English](README.en.md)

> **動作環境: macOS 専用**
> このアプリは **macOS でのみ動作します**。OCR エンジンとして使用している `ndlkotenocr-lite` が Windows に対応していないため、Windows での動作はサポートされません。

影印校エディタは、NDL古典籍OCRなどで生成された日本語縦書き史料のOCRテキストを、研究者が確認・訂正・記録するための校正支援アプリです。
...
影印校エディタは、単にOCRテキストを「きれいに直す」ためのツールではありません。史料画像とOCRテキストを見比べながら、どの文字をどのように読むかという校正判断を蓄積し、史料OCRを研究可能なテキストへ育てるための作業環境です。
...
- **Frontmatter / 本文コンテキストによる候補補強**: ノートの書誌情報（当事者・時代・地名など）と本文から抽出したトークンを候補スコアリングに反映（v1.1.2）
- **候補カテゴリのカラーコーディング**: 辞書由来・字形由来・ファイル文脈由来で色分け表示（v1.1.2）
- 候補選択、手動訂正、スキップ、却下の記録
- **辞書管理GUI**: temporary / approval 辞書をブラウザ上でインライン編集（全フィールド常時編集可・ヘッダーの「💾 保存」で一括保存）
- **手動昇格フロー**: temporary → approval 辞書への昇格を GUI から実行
- 校正ログ・評価ログの出力
- React + Vite frontend / FastAPI backend によるスタンドアロン構成
- **production 起動モード**: `./start.sh --prod` でフロントエンドをビルドし FastAPI から 1 プロセスで配信（v1.1.3）
- **Obsidian Bridge Plugin** （オプション）: Obsidian から「Open in OCR Proofer」で影印校エディタにノートを受け渡す SSE bridge（v1.1.3）
...
├── obsidian-plugin/  # Obsidian bridge plugin (Obsidian → web app へのノート受け渡し)
├── data/             # dictionaries and rule files
└── evaluation/       # logs and evaluation scripts
```

現在のメインUIは、ブラウザで動作するスタンドアロンWebアプリです。

## Obsidian Bridge Plugin

| 項目 | 内容 |
|------|------|
| メインUI | スタンドアロン Web アプリ（`frontend/`） |
| Bridge Plugin | **オプション機能**（`obsidian-plugin/`） |
| 開発の正本 | 本リポジトリに同梱 |
| 配付形式 | `npm run build` で `main.js` / `manifest.json` / `styles.css` / `versions.json` を生成 |

Bridge Plugin を使うと、Obsidian 側でノートを右クリック → 「Open in OCR Proofer」で
起動中の影印校エディタにノートを受け渡せます。
OCR の確認・校正はすべてブラウザ側で行います。

現時点では別リポジトリ・Git submodule としては公開していませんが、
ビルドと配付は独立して完結する構成にしています。

詳細は [`obsidian-plugin/README.md`](obsidian-plugin/README.md) を参照してください。

### Bridge Plugin のインストール（概要）

```bash
cd obsidian-plugin
npm install
npm run build
# 生成された main.js + manifest.json + styles.css を
# <Vault>/.obsidian/plugins/ocr-proofer/ にコピー → Obsidian で有効化
```
...
バックエンド（uvicorn + `--reload`, port 8000）と Vite dev server（port 5176）の 2 プロセスを同時起動します。

```text
http://localhost:5176
```

#### production モード（ビルド済み配信）

```bash
./start.sh --prod
```

`npm run build` でフロントエンドをビルドしたあと、FastAPI（port 8000）が静的ファイルをまとめて配信する **1 プロセス構成**で起動します。`--reload` は無効です。

```text
http://localhost:8000
```

> **注意**: production モードはローカル利用を前提とした構成（`127.0.0.1` バインド）です。LAN 公開・外部公開・systemd / Docker デプロイは現バージョンのスコープ外です。

受け入れ確認手順は [`docs/acceptance_scenarios.md`](docs/acceptance_scenarios.md) を参照してください。

#### 停止・状態確認

```bash
./start.sh --stop    # 起動中のプロセスを停止
./start.sh --status  # ポートとモード（dev / prod）を確認
```
...
- [x] Frontmatter から候補コンテキストを抽出・スコアリングに反映（FM bonus +26pt）
- [x] 本文（非OCR Markdown）から候補コンテキストを抽出・スコアリングに反映（body bonus +15pt）
- [x] FM / body 由来 bonus を proper_noun_terms のみに限定（bonus-only パス）
- [x] span_category（辞書 / 字形 / ファイル文脈）のカラーコーディング
- [x] popover に FM / 本文 ヒントバッジを表示
- [x] WikiLink heading anchor（`[[page#anchor]]`）の stripping（FM・本文両対応）
- [x] `./start.sh --prod`: frontend ビルド + FastAPI 静的配信による 1 プロセス production 起動モード
- [x] **Obsidian Bridge Plugin** (`obsidian-plugin/`): 「Open in OCR Proofer」コマンドで Obsidian から影印校エディタに SSE bridge 経由でノートを受け渡す機能を正式リリース（v1.1.3）
...
- [ ] OCR実行ボタン周辺のUI改善
- [ ] Obsidian plugin: React UI の Obsidian サイドパネルへの埋め込み（bridge は v1.1.3 で実装済み）

## 現在の制限

- **このアプリは macOS 専用です。** `ndlkotenocr-lite` が Windows に対応していないため、Windows での動作はサポートされません。
- 現時点の主UIはスタンドアロンWebアプリです。Obsidian Bridge Plugin（`obsidian-plugin/`）はノートを影印校エディタに受け渡す bridge のみ実装しており、Obsidian サイドパネルへの React UI 埋め込みは将来対応予定です。
...
最新の公開リリースは `v1.1.2` です。`main` ブランチでは v1.1.3 の開発が進行中です（production 起動モード対応など）。
