...
export function logSpanSkipped({ docId, spanId, suspectSpan, isDangerous, popoverOpenMs, dismissTrigger, candidatesViewed }) {
  _append({ event: 'span_skipped', docId, spanId, suspectSpan, isDangerous, popoverOpenMs, dismissTrigger, candidatesViewed });
}
...
export function logSpanRejected({ docId, spanId, suspectSpan, isDangerous, rejectedCandidate, tier }) {
  _append({ event: 'span_rejected', docId, spanId, suspectSpan, isDangerous, reject_target: 'top1', rejected_candidate: rejectedCandidate, tier: tier ?? null });
}
...
export function logSpanAccepted({
  ...
  wasBigramSuspect, particleScript, dismissedDangerFlag, candidateRank, hintSource,
}) {
  const payload = {
    event: 'span_accepted',
    docId,
